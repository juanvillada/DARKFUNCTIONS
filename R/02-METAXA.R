...
phylum.bar <- phylum.bar %>% 
  arrange(match(Phylum, phylum.sum[["All"]] %>% pull(Phylum)))
...
genus.bin.veg.norm <- genus.bin.veg %>% 
  counts(normalized = T) %>% 
  as_tibble

# Get results
genus.bin.veg <- genus.bin.veg %>% 
  results %>% 
  as_tibble
...
# Run models
genus.bin.ch <- genus %>% select(metadata %>% filter(Layer == "organic") %>% pull(Sample)) %>% 
  runDeseq(metadata %>% filter(Layer == "organic"), "CH4")

genus.bin.co <- genus %>% select(metadata %>% filter(Layer == "organic") %>% pull(Sample)) %>% 
  runDeseq(metadata %>% filter(Layer == "organic"), "CO2")

genus.bin.no <- genus %>% select(metadata %>% filter(Layer == "organic") %>% pull(Sample)) %>% 
  runDeseq(metadata %>% filter(Layer == "organic"), "N2O")

# Get normalised counts
genus.bin.ch.norm <- genus.bin.ch %>% 
  counts(normalized = T) %>% 
  as_tibble

genus.bin.co.norm <- genus.bin.co %>% 
  counts(normalized = T) %>% 
  as_tibble

genus.bin.no.norm <- genus.bin.no %>% 
  counts(normalized = T) %>% 
  as_tibble

# Get results
genus.bin.ch <- genus.bin.ch %>% 
  results %>% 
  as_tibble

genus.bin.co <- genus.bin.co %>% 
  results %>% 
  as_tibble

genus.bin.no <- genus.bin.no %>% 
  results %>% 
  as_tibble
